  ...
  return (
    <div style={{maxWidth:720,margin:'0 auto',padding:'28px 20px'}}>
      ...
      <div className="animate-fadeUp" style={{marginBottom:28}}>
        ...
        <p style={{fontSize:14,color:'var(--text-3)',lineHeight:1.6}}>{subGreeting}</p>
      </div>

      {/* ── QUICK ACTION PILLS ── */}
      <div className="animate-fadeUp delay-100" style={{display:'flex',gap:8,marginBottom:28,flexWrap:'wrap'}}>
        {[
          {label:'📍 Nearby',view:'nearby'},
          {label:'🔍 Browse All',view:'browse'},
          {label:'🗺️ Open Map',view:'map'},
        ].map(({label,view})=>(
          <button key={view} onClick={()=>onNavigate(view)} style={{
        ...
        <>
          ...
          <div className="card animate-fadeUp delay-100" style={{padding:'18px 20px',marginBottom:24}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
              <p style={{fontSize:14,color:'var(--text-1)',fontWeight:600,letterSpacing:'-0.1px'}}>Search radius</p>
              <span style={{fontSize:18,fontWeight:700,color:'var(--crema-mid)',fontFamily:"'Cormorant Garamond',serif",letterSpacing:'-0.2px',fontFamily:"'Cormorant Garamond',serif"}}>{radius} mile{radius!==1?'s':''}</span>
            </div>
          ...
          <p style={{fontSize:12,color:'var(--text-3)',marginBottom:14,letterSpacing:'0.2px',fontWeight:500}}><span style={{color:'var(--text-1)',fontWeight:700}}>{shopsWithDistance.length}</span> café{shopsWithDistance.length!==1?'s':''} within {radius} mile{radius!==1?'s':''}</p>
          ...
          <div key={shop.id} className="card animate-fadeUp" style={{marginBottom:16,overflow:'hidden',cursor:'pointer',transition:'border-color 0.2s',animationDelay:`${i*0.06}s`}}
            ...
            <div style={{padding:'16px 20px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                <div>
                  <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.2px'}}>{shop.name}</h3>
                  <p style={{fontSize:12,color:'var(--text-3)',marginTop:3,letterSpacing:'0.1px',letterSpacing:'0.1px'}}>{shop.address} · {shop.neighborhood}</p>
                </div>
                {shop.visited&&<span style={{fontSize:11,background:'var(--crema)',color:'#FAF6F0',borderRadius:20,padding:'3px 10px',fontWeight:700,flexShrink:0}}>✓ Visited</span>}
              </div>
              <p style={{fontSize:13,color:'var(--text-2)',marginTop:10,lineHeight:1.6}}>{shop.description}</p>
              <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:10}}>
                {shop.tags?.map(t=><span key={t} style={{fontSize:10,color:'var(--text-2)',background:'var(--surface-3)',border:'1px solid var(--border)',borderRadius:6,padding:'3px 9px',fontWeight:500}}>{t}</span>)}
              </div>
              <div style={{borderTop:'1px solid var(--border)',marginTop:14,paddingTop:14,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <div style={{display:'flex',gap:14}}>
                  <span style={{fontSize:12,color:'var(--text-3)',letterSpacing:'0.1px'}}>🌿 {amb>0?Math.round(amb):'—'}</span>
