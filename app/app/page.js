          ...
          {greeting}{displayName?`, ${displayName}`:''}.
            {nearestShops.map((shop,i)=>(
              <div key={shop.id} className="animate-fadeUp" style={{animationDelay:`${i*0.06}s`}}>
                <CafeCard shop={shop} avgScores={avgScores} size="compact" onSelect={onSelectShop} onLog={onLog} showDistance={true}/>
              </div>
            ))}
      </div>
...
function Nav({activeView,setActiveView,user,pendingCount,onAuthClick,onSignOut}) {
  ...
  return (
    <>
      ...
      {/* ── Mobile bottom nav ── */}
      <nav className="mobile-nav" style={{position:'fixed',bottom:0,left:0,right:0,height:64,background:'var(--surface)',borderTop:'1px solid var(--border)',display:'none',alignItems:'center',justifyContent:'space-around',zIndex:500,boxShadow:'0 -2px 12px rgba(0,0,0,0.08)',paddingBottom:'env(safe-area-inset-bottom)'}}>
        {tabs.map(tab=>(
          <button key={tab.id} onClick={()=>setActiveView(tab.id)} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:3,background:'none',border:'none',cursor:'pointer',padding:'8px 12px',position:'relative',opacity:activeView===tab.id?1:0.45,transition:'opacity 0.2s'}}>
            ...
            <span style={{fontSize:10,fontWeight:600,color:activeView===tab.id?'var(--crema-mid)':'var(--text-3)',letterSpacing:'0.3px',fontFamily:"'DM Sans',sans-serif",letterSpacing:0.3}}>{tab.label}</span>
            {tab.id==='friends'&&pendingCount>0&&(
            ...
            <div key={shop.id} className="animate-fadeUp" style={{animationDelay:`${i*0.05}s`}}>
              <CafeCard shop={shop} avgScores={avgScores} size="row" onSelect={onSelect} onLog={onLog} showDistance={true}/>
                    ...
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8}}>
                      <div>
                        <div style={{display:'flex',alignItems:'center',gap:6}}>
                          <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:700,color:'var(--text-1)'}}>{shop.name}</h3>
                          {shop.visited&&<span style={{fontSize:10,background:'var(--crema)',color:'#FAF6F0',borderRadius:20,padding:'1px 7px',fontWeight:700,flexShrink:0}}>✓</span>}
                        </div>
                        <p style={{fontSize:11,color:'var(--text-3)',margin:'3px 0 6px',letterSpacing:'0.1px'}}>{shop.neighborhood}</p>
                      </div>
                    </div>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                      <div style={{display:'flex',gap:10}}>
                ...
                <div style={{borderTop:'1px solid var(--border)',padding:'8px 16px',display:'flex',gap:16}}>
                  ...
                  <span style={{fontSize:11,color:'var(--text-3)',letterSpacing:'0.1px'}}>🚗 {Math.round(shop.distanceMiles*3)} min drive</span>
                  {avg>0&&<span style={{fontSize:11,color:'var(--crema-mid)',fontWeight:600,marginLeft:'auto'}}>★ {avg.toFixed(1)} avg</span>}
                </div>
              </div>
            )
          })}
      ...
      <div style={{padding:'14px 16px 16px'}}>
        ...
        <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:10}}>
          {shop.tags?.slice(0,3).map(t=>(
            <span key={t} style={{fontSize:10,color:'var(--text-2)',background:'var(--surface-2)',border:'1px solid var(--border)',borderRadius:6,padding:'2px 8px',fontWeight:500}}>{t}</span>
          ))}
        </div>
        {/* Score row */}
