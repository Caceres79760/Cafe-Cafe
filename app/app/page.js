...
// ─── Hours helpers ────────────────────────────
// hours array: 7 strings index 0=Mon … 6=Sun
// Each entry: "HH:MM-HH:MM" or "closed"
...
function isOpenNow(hours) {
  const todayStr = getTodayHours(hours)
  if (!todayStr || todayStr === 'closed') return false
...
function formatHours(hours) {
  ...
  if (todayStr === 'closed') return 'Closed today'
  const [openStr, closeStr] = todayStr.split('-')
  const fmt = (t) => {
  ...
  return `${fmt(openStr)} – ${fmt(closeStr)}`
}

function HoursBadge({ hours, size = 'sm' }) {
  const open = isOpenNow(hours)
  const todayStr = getTodayHours(hours)
  const isClosed = !todayStr || todayStr === 'closed'
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        background: isClosed ? 'var(--text-3)' : open ? 'var(--success)' : 'var(--error)',
      }}/>
      <span style={{
        color: isClosed ? 'var(--text-3)' : open ? 'var(--success)' : 'var(--error)',
      }}>
        {isClosed ? 'Closed today' : open ? `Open · ${hoursText}` : `Closed · Opens ${hoursText?.split('–')[0]?.trim()}`}
      </span>
    </div>
  )
}
...
// Warm, illustrated empty states for new users.
// Used across Profile, Visits, Friends, and Browse.
function EmptyState({ icon, title, subtitle, action, onAction, secondaryAction, onSecondaryAction }) {
  return (
    <div style={{
      display: 'flex', flexDirection: 'column', alignItems: 'center',
      justifyContent: 'center', textAlign: 'center',
      padding: '56px 32px', gap: 0,
    }}>
      ...
      {action && (
        <button onClick={onAction} className="btn-primary"
          style={{ fontSize: 14, padding: '12px 28px', marginBottom: 12 }}>
          {action}
        </button>
      )}
      ...
      {/* Secondary action */}
      {secondaryAction && (
        <button onClick={onSecondaryAction} style={{
  ...
  return (
    <div style={{marginBottom:12}}>
      <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:6}}>
        ...
        <span style={{fontSize:12,color:'var(--text-2)',flex:1,fontWeight:500,letterSpacing:'0.2px'}}>{label}</span>
        <span style={{fontSize:13,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.1px'}}>{value}<span style={{fontSize:11,fontWeight:400,color:'var(--text-3)' }}>/5</span></span>
      </div>
...
function AuthModal({onClose,onSuccess}) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(180,165,145,0.88)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:9999,padding:24}}
      onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="card animate-scaleIn" style={{width:'100%',maxWidth:400,padding:'32px 28px'}}>
        <div style={{textAlign:'center',marginBottom:28}}>
          <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:30,fontWeight:700,fontStyle:'italic',color:'var(--text-1)',letterSpacing:'0.5px'}}>Café Café</h1>
        ...
        <div style={{display:'flex',background:'var(--surface-3)',borderRadius:10,padding:3,marginBottom:24}}>
          {['login','signup'].map(m=>(
            <button key={m} onClick={()=>{setMode(m);setError('')}} style={{flex:1,padding:'8px',borderRadius:8,border:'none',cursor:'pointer',background:mode===m?'var(--crema-mid)':'transparent',color:mode===m?'var(--bg)':'var(--text-3)',fontWeight:700,fontSize:13,fontWeight:600,transition:'all 0.2s',fontFamily:"'DM Sans',sans-serif",letterSpacing:'0.2px'}}>
...
function LogModal({shop,user,onClose,onSubmit,onNeedAuth}) {
  ...
  if(done) return (
    <div className="animate-scaleIn" style={{position:'fixed',inset:0,background:'rgba(180,165,145,0.92)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:9999}}>
      <div className="card" style={{padding:'48px 40px',textAlign:'center',maxWidth:320}}>
        <div style={{fontSize:56,marginBottom:12}}>☕</div>
        <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:26,color:'var(--text-1)',marginBottom:8}}>Visit Logged!</h2>
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(180,165,145,0.85)',display:'flex',alignItems:'flex-end',justifyContent:'center',zIndex:9999}}
      onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="animate-slideUp" style={{background:'var(--surface)',borderRadius:'20px 20px 0 0',width:'100%',maxWidth:600,maxHeight:'92vh',overflowY:'auto',padding:'0 0 40px',border:'1px solid var(--border)',borderBottom:'none'}}>
        <div style={{display:'flex',alignItems:'center',gap:12,padding:'20px 24px 16px',borderBottom:'1px solid var(--border)'}}>
          ...
          <div style={{flex:1}}>
            <p style={{fontSize:10,color:'var(--crema-mid)',fontWeight:700,letterSpacing:'2px',textTransform:'uppercase'}}>NEW VISIT</p>
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:19,color:'var(--text-1)',fontWeight:700,letterSpacing:'-0.2px'}}>{shop?.name}</h2>
          </div>
        ...
        <div style={{padding:'20px 24px'}}>
          {!user&&(
            <div style={{background:'var(--crema-mid)22',border:'1px solid var(--crema-mid)44',borderRadius:12,padding:'12px 16px',marginBottom:18,display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
              <p style={{fontSize:13,color:'var(--text-1)',lineHeight:1.5}}>Create a free account to save your visits.</p>
              <button onClick={onNeedAuth} className="btn-primary" style={{fontSize:12,padding:'7px 14px',flexShrink:0}}>Sign Up</button>
            </div>
          )}
          <p style={{fontSize:14,color:'var(--text-1)',fontWeight:700,marginBottom:4}}>Rate your visit</p>
          ...
          {CATEGORIES.map(cat=>(
            <div key={cat.key} className="card" style={{padding:18,marginBottom:12}}>
              <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:14}}>
                ...
                <div style={{flex:1}}>
                  <p style={{fontSize:14,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.1px'}}>{cat.label}</p>
          ...
          <div style={{marginTop:4}}>
            <p style={{fontSize:14,color:'var(--text-1)',fontWeight:700,marginBottom:8}}>Add a note <span style={{color:'var(--text-3)',fontWeight:400,fontSize:13}}>(optional)</span></p>
            ...
            <p style={{fontSize:11,color:'var(--text-3)',textAlign:'right',marginTop:6,letterSpacing:'0.2px'}}>{note.length}/280</p>
          </div>
  ...
  return (
    <div className="animate-slideUp" style={{position:'absolute',bottom:0,left:0,right:0,background:'var(--surface)',borderTop:'1px solid var(--border)',borderRadius:'20px 20px 0 0',maxHeight:'72vh',overflowY:'auto',zIndex:1000,padding:'0 0 32px'}}>
      ...
      <div style={{position:'relative',margin:'0 16px',borderRadius:14,overflow:'hidden'}}>
        ...
        {shop.visited&&<div style={{position:'absolute',top:10,right:10,background:'var(--crema)',color:'#FAF6F0',fontSize:10,fontWeight:700,padding:'4px 10px',borderRadius:20,letterSpacing:'0.2px'}}>✓ Visited</div>}
      ...
      <div style={{padding:'16px 20px 0'}}>
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:12}}>
          <div>
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:700,color:'var(--text-1)',lineHeight:1.2,letterSpacing:'-0.2px'}}>{shop.name}</h2>
            <p style={{fontSize:12,color:'var(--text-3)',marginTop:3,letterSpacing:'0.1px'}}>{shop.address} · {shop.neighborhood}</p>
            {shop.hours&&<div style={{marginTop:5}}><HoursBadge hours={shop.hours}/></div>}
          </div>
        ...
        <p style={{fontSize:13,color:'var(--text-2)',lineHeight:1.65,margin:'10px 0 14px'}}>{shop.description}</p>
        <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:16}}>
          {shop.tags?.map(t=><span key={t} style={{fontSize:10,color:'var(--text-2)',background:'var(--surface-3)',border:'1px solid var(--border)',borderRadius:6,padding:'3px 9px',fontWeight:500,letterSpacing:'0.1px'}}>{t}</span>)}
        </div>
        {(amb>0||tas>0||val>0)&&(
          <div style={{borderTop:'1px solid var(--border)',paddingTop:14,marginBottom:14}}>
            <p style={{fontSize:10,color:'var(--text-3)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:12}}>Community Scores</p>
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(180,165,145,0.88)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:9999,padding:24}}
      ...
      <div className="card animate-scaleIn" style={{width:'100%',maxWidth:480,maxHeight:'85vh',overflowY:'auto',padding:0}}>
        ...
        <div style={{padding:'24px 24px 20px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',gap:12}}>
          ...
          <div style={{flex:1}}>
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.2px'}}>@{friend.username}</h2>
            <p style={{fontSize:12,color:'var(--text-3)',marginTop:3,letterSpacing:'0.1px',letterSpacing:'0.1px'}}>{total} visit{total!==1?'s':''} logged</p>
          </div>
          <button onClick={onClose} style={{background:'var(--surface-3)',border:'1px solid var(--border)',borderRadius:8,width:32,height:32,cursor:'pointer',color:'var(--text-2)',fontSize:14}}>✕</button>
        </div>
        ...
        <div style={{padding:'20px 24px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:20}}>
            {[{label:'Visited',value:uniqueShops.length,icon:'📍'},{label:'Avg Taste',value:avgTaste,icon:'☕'},{label:'Avg Vibe',value:avgAmbiance,icon:'🌿'}].map(s=>(
              <div key={s.label} style={{background:'var(--surface-2)',borderRadius:12,padding:'12px 8px',textAlign:'center',border:'1px solid var(--border)'}}>
                ...
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.2px'}}>{s.value}</div>
          ...
          {/* Visited cafés */}
          <p style={{fontSize:10,color:'var(--text-3)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:12}}>Cafés Visited</p>

          {loading?<Spinner/>:visits.length===0?(
            <EmptyState icon="☕" title="No visits yet" subtitle="This friend hasn't logged any café visits yet."/>
          ):visits.map((visit,i)=>{
            ...
            return (
              <div key={visit.id} style={{display:'flex',gap:12,alignItems:'center',padding:'12px 0',borderBottom:'1px solid var(--border)'}}>
                ...
                <div style={{flex:1}}>
                  <h4 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:16,fontWeight:700,color:'var(--text-1)'}}>{shop.name}</h4>
                  <p style={{fontSize:11,color:'var(--text-3)',margin:'3px 0 6px',letterSpacing:'0.1px'}}>{shop.neighborhood} · {new Date(visit.visited_at).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}</p>
...
function FriendsView({user,shops,onNeedAuth}) {
  ...
  if(!user) return (
    <div style={{maxWidth:480,margin:'80px auto',padding:'0 24px',textAlign:'center'}}>
      ...
      <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,color:'var(--text-1)',marginBottom:8}}>Find your coffee crew</h2>
      <p style={{fontSize:13,color:'var(--text-3)',marginBottom:20,letterSpacing:'0.1px'}}>Sign in to add friends and see where they've been.</p>
  ...
  return (
    <div style={{maxWidth:640,margin:'0 auto',padding:'32px 24px'}}>
      <h1 className="animate-fadeUp" style={{fontFamily:"'Cormorant Garamond',serif",fontSize:34,fontWeight:700,color:'var(--text-1)',marginBottom:4,letterSpacing:'-0.4px',lineHeight:1.15}}>Friends</h1>
      <p className="animate-fadeUp delay-100" style={{fontSize:13,color:'var(--text-3)',marginBottom:20,letterSpacing:'0.1px'}}>{friends.length} friend{friends.length!==1?'s':''} · {pendingIn.length} pending request{pendingIn.length!==1?'s':''}</p>
      ...
      {tab==='friends'&&(
        loading?<Spinner/>:friends.length===0?(
          <EmptyState icon="👥" title="No friends yet" subtitle="Add friends to see where they've been drinking coffee. Their visits could lead you to your next favourite spot." action="Find Friends →" onAction={()=>setTab('search')}/>
        ):friends.map((f,i)=>(
          <div key={f.id} className="card animate-fadeUp" style={{padding:'16px 18px',marginBottom:10,display:'flex',alignItems:'center',gap:14,animationDelay:`${i*0.06}s`}}>
            ...
            <div style={{flex:1}}>
              <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.2px'}}>@{f.profile?.username||'unknown'}</h3>
              <p style={{fontSize:11,color:'var(--text-3)',marginTop:3,letterSpacing:'0.1px'}}>Friends since {new Date(f.created_at).toLocaleDateString('en-US',{month:'short',year:'numeric'})}</p>
            </div>
        ...
        <div>
          {pendingIn.length===0&&pendingOut.length===0?(
            ...
            <>
              {pendingIn.length>0&&(
                <>
                  <p style={{fontSize:10,color:'var(--text-3)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:12}}>Incoming Requests</p>
                  {pendingIn.map((f,i)=>(
                    <div key={f.id} className="card animate-fadeUp" style={{padding:'16px 18px',marginBottom:10,display:'flex',alignItems:'center',gap:14,animationDelay:`${i*0.06}s`}}>
                      ...
                      <div style={{flex:1}}>
                        <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.2px'}}>@{f.profile?.username||'unknown'}</h3>
                        <p style={{fontSize:11,color:'var(--success)',fontWeight:500,marginTop:3}}>Wants to be friends</p>
                      </div>
                      <div style={{display:'flex',gap:8}}>
              ...
              {pendingOut.length>0&&(
                <>
                  <p style={{fontSize:10,color:'var(--text-3)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:12,marginTop:pendingIn.length>0?20:0}}>Sent Requests</p>
                  {pendingOut.map((f,i)=>(
                    <div key={f.id} className="card animate-fadeUp" style={{padding:'16px 18px',marginBottom:10,display:'flex',alignItems:'center',gap:14,animationDelay:`${i*0.06}s`}}>
                      ...
                      <div style={{flex:1}}>
                        <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.2px'}}>@{f.profile?.username||'unknown'}</h3>
                        <p style={{fontSize:11,color:'var(--text-3)',fontWeight:500,marginTop:3,fontStyle:'italic'}}>Awaiting response……</p>
                      </div>
                      <button onClick={()=>declineRequest(f.id)} disabled={actionLoading===f.id} style={{background:'none',border:'1px solid var(--border)',borderRadius:10,padding:'7px 12px',fontSize:12,color:'var(--text-3)',cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}}>
        ...
        <div>
          ...
          {!searching&&searchQuery&&searchResults.length===0&&(
            <div className="card" style={{padding:'32px',textAlign:'center'}}>
              <p style={{fontSize:14,color:'var(--text-3)'}}>No users found for "<span style={{color:'var(--text-1)'}}>{searchQuery}</span>"</p>
            </div>
          )}
          {!searching&&searchResults.map((profile,i)=>{
            ...
            return (
              <div key={profile.id} className="card animate-fadeUp" style={{padding:'16px 18px',marginBottom:10,display:'flex',alignItems:'center',gap:14,animationDelay:`${i*0.06}s`}}>
                <div style={{width:44,height:44,borderRadius:14,background:'var(--crema-mid)22',border:'2px solid var(--crema-mid)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,flexShrink:0}}>☕</div>
                <div style={{flex:1}}>
                  <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.2px'}}>@{profile.username}</h3>
          ...
          {!searchQuery&&(
            <EmptyState icon="🔎" title="Find your coffee crew" subtitle="Type a username above to search for friends on Café Café."/>
          )}
        </div>
      )}
...
// ─── Home View ────────────────────────────────
// Personalised dashboard shown on app open.
// Two sections:
//   1. Nearest cafés (location-aware, sorted by distance)
//   2. Friend activity (recent visits from accepted friends)
function HomeView({user,shops,visits,avgScores,userLocation,locationLoading,onRequestLocation,onSelectShop,onLog,onAuthClick,onNavigate,loadingShops}) {
  ...
  useEffect(()=>{
    ...
    const loadFriendActivity=async()=>{
      ...
      // Get their recent visits (last 20, across all friends)
  ...
  const nearestShops=userLocation
    ? [...shops]
        .map(s=>({...s,distanceMiles:getDistanceMiles(userLocation.lat,userLocation.lng,s.lat,s.lng)}))
        .sort((a,b)=>a.distanceMiles-b.distanceMiles)
        .slice(0,6)
    : shops.slice(0,6)

  // ── Time-appropriate sub-greeting ─────────
  const subGreeting=hour<12
    ? 'Ready for your morning cup?'
    : hour<17
    ? 'Time for an afternoon pick-me-up?'
    : 'Finding a cosy spot for the evening?'
  ...
  return (
    <div style={{maxWidth:720,margin:'0 auto',padding:'28px 20px'}}>
      ...
      <div className="animate-fadeUp" style={{marginBottom:28}}>
        ...
        <p style={{fontSize:14,color:'var(--text-3)',lineHeight:1.6,marginTop:4}}>{subGreeting}</p>
      </div>

      {/* ── NEAREST CAFÉS ── */}
      <div className="animate-fadeUp delay-100" style={{marginBottom:32}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline',marginBottom:14}}>
          <div>
            <p style={{fontSize:10,color:'var(--crema-mid)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:3}}>
              {userLocation?'Nearest to you':'Cafés in Rochester'}
            </p>
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.2px'}}>
              {userLocation?'Close by right now':'Start exploring'}
            </h2>
          </div>
          <button onClick={()=>onNavigate('nearby')} style={{background:'none',border:'none',color:'var(--crema-mid)',fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}}>
            See all →
          </button>
        </div>
        ...
        {!userLocation&&!locationLoading&&(
          <div style={{
            ...
            <div style={{flex:1}}>
              <p style={{fontSize:13,fontWeight:600,color:'var(--text-1)',marginBottom:2}}>Enable location for better results</p>
              <p style={{fontSize:12,color:'var(--text-3)'}}>See cafés sorted by how close they are to you right now.</p>
            </div>
            <button onClick={onRequestLocation} className="btn-primary" style={{fontSize:12,padding:'7px 14px',flexShrink:0}}>Enable</button>
          </div>
        )}

        {locationLoading&&(
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:14,padding:'20px',textAlign:'center',marginBottom:14}}>
            <div className="animate-spin" style={{width:24,height:24,border:'2px solid var(--border)',borderTopColor:'var(--crema-mid)',borderRadius:'50%',margin:'0 auto'}}/>
          </div>
        )}

        {/* Horizontal scrolling café strip */}
        {loadingShops?<Spinner/>:(
          <div style={{display:'flex',gap:12,overflowX:'auto',paddingBottom:8,scrollbarWidth:'none'}}>
            {nearestShops.map((shop,i)=>{
              ...
              return (
                <div key={shop.id} className="animate-fadeUp" style={{
                ...
                onMouseEnter={e=>{e.currentTarget.style.borderColor='var(--crema-mid)';e.currentTarget.style.transform='translateY(-2px)'}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor='var(--border)';e.currentTarget.style.transform='translateY(0)'}}
                onClick={()=>onSelectShop(shop)}>
                  {/* Image */}
                  <div style={{position:'relative'}}>
                    <img src={shop.image_url} alt={shop.name} style={{width:'100%',height:110,objectFit:'cover',display:'block'}}/>
                    {shop.visited&&(
                      <div style={{position:'absolute',top:8,right:8,background:'var(--crema-mid)',color:'#FAF6F0',fontSize:9,fontWeight:700,padding:'2px 8px',borderRadius:20,letterSpacing:'0.2px'}}>✓ VISITED</div>
                    )}
                    {shop.distanceMiles!==undefined&&(
                      <div style={{position:'absolute',bottom:8,left:8,background:'rgba(237,232,220,0.92)',color:'var(--text-1)',fontSize:10,fontWeight:600,padding:'2px 8px',borderRadius:20,backdropFilter:'blur(4px)'}}>
                  ...
                  <div style={{padding:'11px 13px'}}>
                    <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,fontWeight:700,color:'var(--text-1)',marginBottom:2,lineHeight:1.2,letterSpacing:'-0.1px'}}>{shop.name}</h3>
                    <p style={{fontSize:11,color:'var(--text-3)',marginBottom:4}}>{shop.neighborhood}</p>
                    {shop.hours&&<div style={{marginBottom:6}}><HoursBadge hours={shop.hours}/></div>}
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                      {avg>0?<Stars value={Math.round(avg)} size={12}/>:<p style={{fontSize:11,color:'var(--text-3)'}}>No ratings yet</p>}
                      <button onClick={e=>{e.stopPropagation();onLog(shop)}} style={{
                        background:'var(--crema-mid)',color:'#FAF6F0',
      ...
      {/* ── FRIEND ACTIVITY ── */}
      <div className="animate-fadeUp delay-200" style={{marginBottom:32}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline',marginBottom:14}}>
          <div>
            <p style={{fontSize:10,color:'var(--crema-mid)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:3}}>Friend Activity</p>
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.2px'}}>Where friends have been</h2>
          </div>
          <button onClick={()=>onNavigate('friends')} style={{background:'none',border:'none',color:'var(--crema-mid)',fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}}>
            Friends →
          </button>
        </div>
        ...
        {!user&&(
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:14,padding:'24px',textAlign:'center'}}>
            ...
            <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:700,color:'var(--text-1)',marginBottom:6}}>Sign in to see friend activity</p>
            <p style={{fontSize:13,color:'var(--text-3)',marginBottom:16,lineHeight:1.6}}>Create an account to add friends and see where they've been drinking coffee.</p>
            <button onClick={onAuthClick} className="btn-primary" style={{fontSize:13,padding:'9px 22px'}}>Sign In / Sign Up</button>
          </div>
        )}

        {/* Loading */}
        {user&&loadingFriends&&<Spinner/>}

        {/* No friends yet */}
        {user&&!loadingFriends&&friendVisits.length===0&&(
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:14,padding:'24px',textAlign:'center'}}>
            <p style={{fontSize:24,marginBottom:10}}>☕</p>
            <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:700,color:'var(--text-1)',marginBottom:6}}>No friend activity yet</p>
            <p style={{fontSize:13,color:'var(--text-3)',marginBottom:16,lineHeight:1.6}}>Add friends to see where they've been. Their visits might lead you somewhere great.</p>
            <button onClick={()=>onNavigate('friends')} className="btn-primary" style={{fontSize:13,padding:'9px 22px'}}>Find Friends →</button>
          </div>
        )}
        ...
        {user&&!loadingFriends&&friendVisits.length>0&&(
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            {friendVisits.slice(0,8).map((visit,i)=>{
              const shop=shopMap[visit.shop_id]
              const profile=friendProfiles[visit.user_id]
              ...
              return (
                <div key={visit.id} className="animate-fadeUp" style={{
                ...
                onClick={()=>onSelectShop(shop)}>
                  {/* Shop image */}
                  <img src={shop.image_url} alt={shop.name} style={{width:52,height:52,borderRadius:10,objectFit:'cover',flexShrink:0}}/>
                  {/* Info */}
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:3}}>
                      {/* Friend avatar */}
                      <div style={{width:20,height:20,borderRadius:6,background:'var(--crema-mid)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,color:'#FAF6F0',fontWeight:700,flexShrink:0}}>
                        {profile.username?.[0]?.toUpperCase()||'?'}
                      </div>
                      <p style={{fontSize:12,fontWeight:600,color:'var(--text-2)'}}>
                        <span style={{color:'var(--crema-mid)'}}>@{profile.username}</span> visited
                      </p>
                      <p style={{fontSize:11,color:'var(--text-3)',marginLeft:'auto',flexShrink:0}}>{timeAgo(visit.visited_at)}</p>
                    </div>
                    <h4 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:16,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.1px',marginBottom:3,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{shop.name}</h4>
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      <Stars value={avg} size={12}/>
                      {visit.note&&<p style={{fontSize:11,color:'var(--text-3)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontStyle:'italic'}}>"{visit.note}"</p>}
                    </div>
                  </div>
                  {/* Log button */}
                  <button onClick={e=>{e.stopPropagation();onLog(shop)}} style={{
                  ...
                  onMouseEnter={e=>{e.currentTarget.style.borderColor='var(--crema-mid)';e.currentTarget.style.color='var(--crema-mid)'}}
      ...
      {/* ── YOUR STATS STRIP (only if logged in and has visits) ── */}
      {user&&visits.length>0&&(
        <div className="animate-fadeUp delay-300" style={{marginBottom:16}}>
          <p style={{fontSize:10,color:'var(--crema-mid)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:14}}>Your Coffee Journey</p>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
            {[
              {label:'Cafés visited',value:new Set(visits.map(v=>v.shop_id)).size,icon:'📍'},
              {label:'Total visits',value:visits.length,icon:'☕'},
              {label:'Avg taste',value:(visits.reduce((a,v)=>a+v.taste_score,0)/visits.length).toFixed(1),icon:'⭐'},
            ].map(stat=>(
              <div key={stat.label} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:14,padding:'14px 12px',textAlign:'center',cursor:'pointer',transition:'border-color 0.2s'}}
                onMouseEnter={e=>e.currentTarget.style.borderColor='var(--crema-mid)'}
                onMouseLeave={e=>e.currentTarget.style.borderColor='var(--border)'}
                onClick={()=>onNavigate('profile')}>
                <div style={{fontSize:20,marginBottom:6}}>{stat.icon}</div>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:26,fontWeight:700,color:'var(--text-1)',lineHeight:1,letterSpacing:'-0.4px'}}>{stat.value}</div>
                <div style={{fontSize:10,color:'var(--text-3)',marginTop:4,fontWeight:500}}>{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <>
      ...
      <nav style={{height:56,background:'var(--surface)',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 16px',flexShrink:0,gap:8}}>
        <div style={{display:'flex',alignItems:'center',gap:6,flexShrink:0}}>
          <span className="nav-wordmark-full" style={{fontFamily:"'Cormorant Garamond',serif",fontSize:21,fontWeight:700,fontStyle:'italic',color:'var(--text-1)',letterSpacing:'0.5px'}}>Café Café</span>
          <span className="nav-wordmark-short" style={{display:'none',fontFamily:"'Cormorant Garamond',serif",fontSize:21,fontWeight:700,fontStyle:'italic',color:'var(--text-1)',letterSpacing:'0.5px'}}>cc</span>
        ...
        <div className="desktop-tabs" style={{display:'flex',gap:2}}>
          {tabs.map(tab=>(
            <button key={tab.id} onClick={()=>setActiveView(tab.id)} style={{background:activeView===tab.id?'var(--crema-mid)':'transparent',color:activeView===tab.id?'#FAF6F0':'var(--text-3)',border:'none',borderRadius:8,padding:'7px 14px',fontSize:13,fontWeight:600,cursor:'pointer',transition:'all 0.2s',fontFamily:"'DM Sans',sans-serif",position:'relative',letterSpacing:'0.1px'}}
            onMouseEnter={e=>{if(activeView!==tab.id){e.currentTarget.style.background='var(--surface-2)';e.currentTarget.style.color='var(--text-1)'}}}
            onMouseLeave={e=>{if(activeView!==tab.id){e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--text-3)'}}}>
              {tab.label}
              {tab.id==='friends'&&pendingCount>0&&(
                <span style={{position:'absolute',top:4,right:4,width:7,height:7,borderRadius:4,background:'var(--error)',border:'2px solid var(--surface)'}}/>
              )}
            </button>
          ))}
        </div>
        {user?(
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            <button onClick={onSignOut} style={{background:'none',border:'1px solid var(--border)',borderRadius:8,padding:'5px 12px',fontSize:12,color:'var(--text-2)',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",flexShrink:0,transition:'all 0.2s'}}
            onMouseEnter={e=>{e.currentTarget.style.borderColor='var(--crema-mid)';e.currentTarget.style.color='var(--text-1)'}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor='var(--border)';e.currentTarget.style.color='var(--text-2)'}}>Sign Out</button>
          </div>
        ):(
      ...
      {/* ── Mobile bottom nav ── */}
      <nav className="mobile-nav" style={{position:'fixed',bottom:0,left:0,right:0,height:60,background:'var(--surface)',borderTop:'1px solid var(--border)',display:'none',alignItems:'center',justifyContent:'space-around',zIndex:500,boxShadow:'0 -2px 12px rgba(0,0,0,0.06)',paddingBottom:'env(safe-area-inset-bottom)'}}>
        {tabs.map(tab=>(
          <button key={tab.id} onClick={()=>setActiveView(tab.id)} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:0,background:'none',border:'none',cursor:'pointer',padding:'6px 8px',position:'relative',flex:1}}>
            {activeView===tab.id&&(
              <div style={{position:'absolute',top:0,left:'50%',transform:'translateX(-50%)',width:24,height:2,borderRadius:1,background:'var(--crema-mid)'}}/>
            )}
            <span style={{fontSize:12,fontWeight:activeView===tab.id?700:500,color:activeView===tab.id?'var(--crema-mid)':'var(--text-3)',letterSpacing:'0.1px',fontFamily:"'DM Sans',sans-serif",marginTop:6,transition:'color 0.15s'}}>{tab.label}</span>
            {tab.id==='friends'&&pendingCount>0&&(
              <span style={{position:'absolute',top:4,right:'calc(50% - 16px)',width:7,height:7,borderRadius:4,background:'var(--error)',border:'2px solid var(--surface)'}}/>
            )}
          </button>
        ))}
      </nav>
...
function NearbyView({shops,avgScores,userLocation,locationError,locationLoading,onRequestLocation,onSelect,onLog}) {
  ...
  return (
    <div style={{maxWidth:680,margin:'0 auto',padding:'32px 24px'}}>
      <h1 className="animate-fadeUp" style={{fontFamily:"'Cormorant Garamond',serif",fontSize:34,fontWeight:700,color:'var(--text-1)',marginBottom:4,letterSpacing:'-0.4px',lineHeight:1.15}}>Nearby</h1>
      <p className="animate-fadeUp delay-100" style={{fontSize:13,color:'var(--text-3)',marginBottom:20,letterSpacing:'0.1px'}}>{userLocation?`Cafés within ${radius} mile${radius!==1?'s':''} of you`:'Find cafés near your current location'}</p>
      {!userLocation&&!locationLoading&&(
        <div className="card animate-fadeUp delay-100" style={{padding:'32px',textAlign:'center',marginBottom:24}}>
          <div style={{fontSize:44,marginBottom:12}}>📍</div>
          <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,color:'var(--text-1)',marginBottom:8}}>{locationError?'Location unavailable':'Share your location'}</h3>
        ...
        <>
          <div className="animate-fadeUp delay-100" style={{display:'flex',gap:8,marginBottom:20,flexWrap:'wrap'}}>
            {RADIUS_OPTIONS.map(r=>(
              <button key={r} onClick={()=>setRadius(r)} style={{background:radius===r?'var(--crema-mid)':'var(--surface)',color:radius===r?'#FAF6F0':'var(--text-2)',border:'1px solid var(--border)',borderRadius:20,padding:'6px 14px',fontSize:13,fontWeight:600,cursor:'pointer',transition:'all 0.2s',fontFamily:"'DM Sans',sans-serif"}}>{r<1?`${r*1000}m`:`${r} mi`}</button>
            ))}
          </div>
          {shopsWithDistance.length===0?(
            <EmptyState
              icon="📍"
              title="No cafés in range"
              subtitle="There are no cafés within this distance. Try increasing the search radius."
            />
          ):shopsWithDistance.map((shop,i)=>(
            <div key={shop.id} className="animate-fadeUp" style={{animationDelay:`${i*0.05}s`}}>
              <CafeCard shop={shop} avgScores={avgScores} size="row" onSelect={onSelect} onLog={onLog} showDistance={true}/>
            </div>
          ))}
        </>
      )}
...
// ─── Shared CaféCard component ───────────────
// One consistent card template used across Browse,
// Nearby, and Home. Accepts a `size` prop:
//   'full'    → tall hero image, full description (Browse)
//   'compact' → smaller image on left, no description (Nearby/Home horizontal)
//   'row'     → image left, info right (Nearby list)
function CafeCard({ shop, avgScores, size='full', onSelect, onLog, showDistance=false }) {
  const amb = avgScores?.[shop.id]?.ambiance ?? 0
  const tas = avgScores?.[shop.id]?.taste    ?? 0
  const val = avgScores?.[shop.id]?.value    ?? 0
  const avg = amb > 0 ? (amb + tas + val) / 3 : 0
  ...
  // ── FULL card (Browse) ──
  if (size === 'full') return (
    <div className="cafe-card-full" style={{
      background:'var(--surface)', border:'1px solid var(--border)',
      borderRadius:18, overflow:'hidden',
      cursor:'pointer', transition:'border-color 0.2s, box-shadow 0.2s',
      marginBottom:14,
    }}
    onMouseEnter={e=>{e.currentTarget.style.borderColor='var(--crema-mid)';e.currentTarget.style.boxShadow='0 4px 20px rgba(124,92,53,0.12)'}}
    ...
    onClick={()=>onSelect(shop)}>
      ...
      <div style={{position:'relative'}}>
        ...
        {/* Visited badge */}
        {shop.visited&&(
          <div style={{position:'absolute',top:10,right:10,background:'var(--crema-mid)',color:'#FAF6F0',fontSize:10,fontWeight:700,padding:'3px 10px',borderRadius:20,letterSpacing:'0.3px'}}>✓ VISITED</div>
        )}
        {/* Hours badge overlaid on image bottom */}
        {shop.hours&&(
          <div style={{position:'absolute',bottom:0,left:0,right:0,background:'linear-gradient(transparent,rgba(44,36,22,0.55))',padding:'18px 14px 10px',display:'flex',alignItems:'flex-end',justifyContent:'space-between'}}>
            <HoursBadge hours={shop.hours} size="sm"/>
          </div>
        )}
      </div>
      {/* Card body */}
      <div style={{padding:'14px 16px 16px'}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8,marginBottom:4}}>
          <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.2px',lineHeight:1.2}}>{shop.name}</h3>
          <button onClick={e=>{e.stopPropagation();onLog(shop)}} style={{
            background:'var(--crema-mid)',color:'#FAF6F0',
          ...
          onMouseLeave={e=>e.currentTarget.style.background='var(--crema-mid)'}>
            Log Visit
          </button>
        </div>
        {/* Meta row */}
        <p style={{fontSize:12,color:'var(--text-3)',marginBottom:8,letterSpacing:'0.1px'}}>{shop.neighborhood}</p>
        {/* Description */}
        {shop.description&&(
          <p style={{
        ...
        <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:10}}>
          {shop.tags?.slice(0,3).map(t=>(
            <span key={t} style={{fontSize:10,color:'var(--text-2)',background:'var(--surface-3)',border:'1px solid var(--border)',borderRadius:6,padding:'2px 8px',fontWeight:500}}>{t}</span>
          ))}
        </div>
        {/* Score row */}
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',paddingTop:10,borderTop:'1px solid var(--border)'}}>
          ...
          {avg>0&&<Stars value={Math.round(avg)} size={13}/>}
        </div>
      </div>
    </div>
  )

  // ── ROW card (Nearby list) ──
  if (size === 'row') return (
    <div style={{
      background:'var(--surface)',border:'1px solid var(--border)',
      borderRadius:16,overflow:'hidden',
    ...
    onMouseEnter={e=>{e.currentTarget.style.borderColor='var(--crema-mid)';e.currentTarget.style.boxShadow='0 2px 12px rgba(124,92,53,0.1)'}}
    onMouseLeave={e=>{e.currentTarget.style.borderColor='var(--border)';e.currentTarget.style.boxShadow='none'}}
    onClick={()=>onSelect(shop)}>
      <div style={{display:'flex'}}>
        ...
        <div style={{position:'relative',flexShrink:0}}>
          ...
          {showDistance&&shop.distanceMiles!==undefined&&(
            <div style={{position:'absolute',bottom:6,left:6,background:'rgba(237,232,220,0.92)',color:'var(--text-1)',fontSize:10,fontWeight:600,padding:'2px 7px',borderRadius:20,backdropFilter:'blur(4px)'}}>
              {formatDistance(shop.distanceMiles)}
            </div>
          )}
        </div>
        ...
        <div style={{padding:'12px 14px',flex:1,minWidth:0}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8,marginBottom:2}}>
            <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.1px',lineHeight:1.2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{shop.name}</h3>
            {shop.visited&&<span style={{fontSize:9,background:'var(--crema-mid)',color:'#FAF6F0',borderRadius:20,padding:'2px 7px',fontWeight:700,flexShrink:0,letterSpacing:'0.2px'}}>✓</span>}
          </div>
          <p style={{fontSize:11,color:'var(--text-3)',marginBottom:5,letterSpacing:'0.1px'}}>{shop.neighborhood}</p>
          {shop.hours&&<div style={{marginBottom:5}}><HoursBadge hours={shop.hours}/></div>}
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <div style={{display:'flex',gap:8}}>
            ...
            <button onClick={e=>{e.stopPropagation();onLog(shop)}} style={{
              background:'var(--crema-mid)',color:'#FAF6F0',
              border:'none',borderRadius:7,padding:'4px 11px',
      ...
      {/* Walk/drive footer */}
      {showDistance&&shop.distanceMiles!==undefined&&(
        <div style={{borderTop:'1px solid var(--border)',padding:'7px 14px',display:'flex',gap:14}}>
          <span style={{fontSize:11,color:'var(--text-3)',letterSpacing:'0.1px'}}>🚶 {Math.round(shop.distanceMiles*20)} min walk</span>
          <span style={{fontSize:11,color:'var(--text-3)',letterSpacing:'0.1px'}}>🚗 {Math.round(Math.max(1,shop.distanceMiles*3))} min drive</span>
          {avg>0&&<span style={{fontSize:11,color:'var(--crema-mid)',fontWeight:600,marginLeft:'auto'}}>★ {avg.toFixed(1)}</span>}
        </div>
      )}
    </div>
  )

  // ── COMPACT card (Home horizontal strip) ──
  return (
    <div style={{
      background:'var(--surface)',border:'1px solid var(--border)',
      borderRadius:16,overflow:'hidden',
      minWidth:196,maxWidth:210,flexShrink:0,
      cursor:'pointer',transition:'border-color 0.2s, transform 0.2s, box-shadow 0.2s',
    }}
    onMouseEnter={e=>{e.currentTarget.style.borderColor='var(--crema-mid)';e.currentTarget.style.transform='translateY(-2px)';e.currentTarget.style.boxShadow='0 6px 20px rgba(124,92,53,0.12)'}}
    ...
    onClick={()=>onSelect(shop)}>
      ...
      <div style={{position:'relative'}}>
        ...
        {shop.visited&&(
          <div style={{position:'absolute',top:7,right:7,background:'var(--crema-mid)',color:'#FAF6F0',fontSize:9,fontWeight:700,padding:'2px 7px',borderRadius:20,letterSpacing:'0.2px'}}>✓</div>
        )}
        {showDistance&&shop.distanceMiles!==undefined&&(
          <div style={{position:'absolute',bottom:7,left:7,background:'rgba(237,232,220,0.92)',color:'var(--text-1)',fontSize:10,fontWeight:600,padding:'2px 7px',borderRadius:20,backdropFilter:'blur(4px)'}}>
            📍 {formatDistance(shop.distanceMiles)}
          </div>
        )}
      </div>
      ...
      <div style={{padding:'10px 12px 12px'}}>
        <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.1px',lineHeight:1.2,marginBottom:2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{shop.name}</h3>
        <p style={{fontSize:11,color:'var(--text-3)',marginBottom:6,letterSpacing:'0.1px'}}>{shop.neighborhood}</p>
        {shop.hours&&<div style={{marginBottom:7}}><HoursBadge hours={shop.hours}/></div>}
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          {avg>0?<Stars value={Math.round(avg)} size={12}/>:<p style={{fontSize:10,color:'var(--text-3)'}}>No ratings yet</p>}
          <button onClick={e=>{e.stopPropagation();onLog(shop)}} style={{
            background:'var(--crema-mid)',color:'#FAF6F0',
...
function BrowseView({shops,avgScores,onSelect,onLog}) {
  ...
  const filtered=shops.filter(shop=>{
    ...
    return(
      ...
      (!openNowOnly||isOpenNow(shop.hours))
    )
  })

  return (
    <div className="content-max-width" style={{maxWidth:680,margin:'0 auto',padding:'24px 24px'}}>
      <h1 className="animate-fadeUp" style={{fontFamily:"'Cormorant Garamond',serif",fontSize:34,fontWeight:700,color:'var(--text-1)',marginBottom:6}}>Browse</h1>
      ...
      {/* Filter pills — visited + open now */}
      <div className="animate-fadeUp delay-100" style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap',alignItems:'center'}}>
        {[['all','All'],['visited','Visited ✓'],['unvisited','Unvisited']].map(([val,label])=>(
          <button key={val} onClick={()=>setFilter(val)} style={{background:filter===val?'var(--crema-mid)':'var(--surface)',color:filter===val?'#FAF6F0':'var(--text-2)',border:'1px solid var(--border)',borderRadius:20,padding:'6px 14px',fontSize:13,fontWeight:600,cursor:'pointer',transition:'all 0.2s',fontFamily:"'DM Sans',sans-serif"}}>{label}</button>
        ))}
        {/* Open Now toggle */}
        <button onClick={()=>setOpenNowOnly(o=>!o)} style={{
          background: openNowOnly ? 'var(--success)' : 'var(--surface)',
          color:      openNowOnly ? '#FAF6F0'        : 'var(--text-2)',
          border: `1px solid ${openNowOnly ? 'var(--success)' : 'var(--border)'}`,
          ...
          <span style={{width:7,height:7,borderRadius:'50%',background:openNowOnly?'#FAF6F0':'var(--success)',flexShrink:0}}/>
          Open Now
        </button>
      </div>
      ...
      {showFilters&&(
        <div className="card animate-fadeUp" style={{padding:'16px 18px',marginBottom:16}}>
          <p style={{fontSize:10,color:'var(--text-3)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:12}}>Advanced Filters</p>
      ...
      {/* Results count */}
      <p style={{fontSize:12,color:'var(--text-3)',marginBottom:14,letterSpacing:'0.2px',fontWeight:500}}>
        <span style={{color:'var(--text-1)',fontWeight:700}}>{filtered.length}</span> result{filtered.length!==1?'s':''}
        {query&&<span> for "<span style={{color:'var(--text-1)'}}>{query}</span>"</span>}
      </p>

      {/* Café cards */}
      {filtered.length===0?(
        <EmptyState icon="🔍" title="No cafés found" subtitle="Try a different search term, or clear your filters to see all cafés in Rochester." action="Clear filters" onAction={()=>{setQuery('');setFilter('all');setNeighborhood('all');setTag('all');setMinScore(0)}}/>
      ):filtered.map((shop,i)=>(
        <div key={shop.id} className="animate-fadeUp" style={{animationDelay:`${i*0.06}s`}}>
          <CafeCard shop={shop} avgScores={avgScores} size="full" onSelect={onSelect} onLog={onLog}/>
        </div>
      ))}
    </div>
  )
...
function ProfileView({user,visits,shops,loading,onAuthClick}) {
  ...
  if(!user) return (
    <div style={{maxWidth:480,margin:'80px auto',padding:'0 24px',textAlign:'center'}}>
      <div style={{fontSize:52,marginBottom:16}}>☕</div>
      <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,color:'var(--text-1)',marginBottom:8}}>Your coffee story starts here</h2>
      <p style={{fontSize:13,color:'var(--text-3)',marginBottom:20,letterSpacing:'0.1px'}}>Sign in to track your visits and build your coffee profile.</p>
  ...
  const P={bg:'var(--bg)',surface:'var(--surface)',border:'var(--border)',espresso:'var(--text-1)',roast:'var(--text-2)',crema:'var(--crema-mid)',slate:'var(--text-3)'}
    ...
    <div style={{background:P.bg,minHeight:'100%'}}>
      <div style={{maxWidth:640,margin:'0 auto',padding:'40px 24px 60px'}}>
        ...
        <div className="animate-fadeUp" style={{marginBottom:28}}>
          ...
          <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:34,fontWeight:700,color:'var(--text-1)',marginBottom:4,letterSpacing:'-0.4px',lineHeight:1.15}}>Your Profile</h1>
          <p style={{fontSize:12,color:'var(--text-3)',letterSpacing:'0.1px'}}>{user.email}</p>
        </div>
          ...
          <>
            ...
            <div className="animate-fadeUp" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:20}}>
              {[{label:'Cafés Visited',value:total,icon:'📍'},{label:'Avg Taste',value:avgTaste,icon:'☕'},{label:'Avg Ambiance',value:avgAmbiance,icon:'🌿'}].map(s=>(
                <div key={s.label} style={{background:P.surface,border:`1px solid ${P.border}`,borderRadius:16,padding:'18px 12px',textAlign:'center'}}>
                  ...
                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:30,fontWeight:700,color:'var(--text-1)',lineHeight:1,letterSpacing:'-0.5px'}}>{s.value}</div>
            ...
            <div className="animate-fadeUp delay-100" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:28}}>
              <div style={{background:P.surface,border:`1px solid ${P.border}`,borderRadius:16,padding:'18px 16px'}}>
                <p style={{fontSize:11,color:'var(--text-3)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:10}}>Avg Value Score</p>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:30,fontWeight:700,color:'var(--text-1)',lineHeight:1,letterSpacing:'-0.5px'}}>{avgValue}</div>
              ...
              <div style={{background:P.surface,border:`1px solid ${P.border}`,borderRadius:16,padding:'18px 16px'}}>
                <p style={{fontSize:11,color:'var(--text-3)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:10}}>Unique Cafés</p>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:30,fontWeight:700,color:'var(--text-1)',lineHeight:1,letterSpacing:'-0.5px'}}>{Object.keys(visitCounts).length}</div>
                <p style={{fontSize:11,color:'var(--text-3)',marginTop:6}}>of {shops.length} in Rochester</p>
              </div>
            </div>
            ...
            {favShop&&(
              <div className="animate-fadeUp delay-200" style={{marginBottom:28}}>
                <p style={{fontSize:11,color:'var(--text-3)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:12}}>⭐ Favourite Café</p>
                <div style={{background:P.surface,border:`1px solid ${P.border}`,borderRadius:18,overflow:'hidden'}}>
                  ...
                  <div style={{padding:'16px 18px'}}>
                    <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.2px'}}>{favShop.name}</h3>
                    <p style={{fontSize:12,color:'var(--text-3)',marginTop:3,letterSpacing:'0.1px'}}>{favShop.neighborhood} · visited {visitCounts[favId]}x</p>
                    <div style={{display:'flex',gap:6,marginTop:10,flexWrap:'wrap'}}>{favShop.tags?.map(t=><span key={t} style={{fontSize:11,color:'var(--text-2)',background:P.crema+'25',border:`1px solid ${P.crema}44`,borderRadius:6,padding:'3px 9px'}}>{t}</span>)}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            {topShop&&topShop.id!==favShop?.id&&(
              <div className="animate-fadeUp delay-300" style={{marginBottom:28}}>
                <p style={{fontSize:11,color:'var(--text-3)',fontWeight:700,letterSpacing:'1.8px',textTransform:'uppercase',marginBottom:12}}>🏆 Highest Rated by You</p>
                <div style={{background:P.surface,border:`1px solid ${P.border}`,borderRadius:16,padding:16,display:'flex',gap:14,alignItems:'center'}}>
                  ...
                  <div>
                    <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.1px'}}>{topShop.name}</h3>
                    <p style={{fontSize:12,color:'var(--text-3)',marginTop:3,letterSpacing:'0.1px'}}>{topShop.neighborhood}</p>
            ...
            {visits.length===0&&(
              <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:18,overflow:'hidden'}}>
                <EmptyState
                  ...
                  subtitle="Every café you visit becomes part of your personal coffee story."
                  action="Find a Café →"
                  onAction={()=>window.dispatchEvent(new CustomEvent('navigate',{detail:'browse'}))}
                />
              </div>
            )}
          </>
          ...
          loading?<Spinner/>:visits.length===0?(
            <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:18,overflow:'hidden'}}>
              <EmptyState
                ...
                subtitle="Head to the map, find a café near you, and log your first visit. It only takes 20 seconds."
                action="Open the Map →"
                onAction={()=>window.dispatchEvent(new CustomEvent('navigate',{detail:'map'}))}
              />
            </div>
          ):(
                    ...
                    <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                          <div>
                            <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.1px'}}>{shop.name}</h3>
                        ...
                        {visit.note&&<p style={{fontSize:12,color:'var(--text-2)',marginTop:8,fontStyle:'italic',lineHeight:1.6}}>"{visit.note}"</p>}
                      </div>
                    </div>
                  </div>
                )
...
function UsernameSetup({user, onComplete}) {
  ...
  return (
    <div style={{height:'100vh',background:'var(--bg)',display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
      <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:24,width:'100%',maxWidth:420,padding:'40px 32px'}}>

        {/* Icon */}
        <div style={{width:64,height:64,borderRadius:20,background:'var(--crema-mid)22',border:'2px solid #c8965a',display:'flex',alignItems:'center',justifyContent:'center',fontSize:28,marginBottom:24}}>☕</div>

        {/* Heading */}
        <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:30,fontWeight:700,color:'var(--text-1)',marginBottom:8,lineHeight:1.15,letterSpacing:'-0.3px'}}>
          One last thing
        </h1>
        ...
        <div style={{position:'relative',marginBottom:8}}>
          <span style={{position:'absolute',left:14,top:'50%',transform:'translateY(-50%)',fontSize:14,color:'var(--text-3)',fontWeight:600}}>@</span>
          <input
            style={{width:'100%',background:'var(--surface-2)',border:`1.5px solid ${error?'var(--error)':'var(--border)'}`,borderRadius:12,padding:'13px 14px 13px 30px',color:'var(--text-1)',fontSize:15,outline:'none',fontFamily:"'DM Sans',sans-serif",transition:'border-color 0.2s',boxSizing:'border-box'}}
            type="text"
        ...
        {/* Character count */}
        <p style={{fontSize:11,color:'var(--text-3)',marginBottom:16,textAlign:'right'}}>{username.length}/20 · lowercase, numbers, underscores only</p>

        {/* Error */}
        {error&&(
          <div style={{background:'var(--error)18',border:'1px solid var(--error)44',borderRadius:10,padding:'10px 14px',marginBottom:16}}>
        ...
        {username.length>=3&&!error&&(
          <div style={{background:'#c8965a12',border:'1px solid #c8965a33',borderRadius:10,padding:'10px 14px',marginBottom:16,display:'flex',alignItems:'center',gap:8}}>
            ...
            <p style={{fontSize:13,color:'var(--crema-mid)'}}>Your profile will show as <strong>@{username}</strong></p>
          </div>
        )}
        ...
        <button
          ...
          style={{width:'100%',background:username.length>=3?'var(--crema-mid)':'var(--surface-2)',color:username.length>=3?'#FAF6F0':'var(--text-3)',border:'none',borderRadius:12,padding:'15px',fontWeight:700,fontSize:15,cursor:username.length>=3&&!loading?'pointer':'default',fontFamily:"'DM Sans',sans-serif",transition:'all 0.2s',opacity:loading?0.7:1}}>
          {loading?'Setting your username…':'Confirm Username →'}
        </button>

        {/* Disclaimer */}
        <p style={{fontSize:11,color:'var(--text-3)',marginTop:14,textAlign:'center',lineHeight:1.6}}>
...
export default function Home() {
  ...
  useEffect(()=>{
    if((activeView==='nearby'||activeView==='home')&&!userLocation&&!locationLoading) requestLocation()
  },[activeView])
  ...
  // Listen for navigation events fired by EmptyState buttons
  useEffect(()=>{
  ...
  return (
    <div style={{height:'100vh',display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{flex:1,overflow:'hidden',position:'relative',paddingBottom:'env(safe-area-inset-bottom)'}}>
        ...
        {activeView==='home'&&(
          <div style={{overflowY:'auto',height:'100%',paddingBottom:64}}>
            <HomeView
              user={user}
              shops={shops}
              visits={visits}
              ...
              onSelectShop={(shop)=>{setSelected(shop);setActiveView('map')}}
              onLog={handleLog}
        ...
        {activeView==='map'&&(
          <div style={{display:'flex',height:'100%'}}>
                      ...
                      <div style={{display:'flex',gap:10,alignItems:'center'}}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:'flex',alignItems:'center',gap:5}}>
                            <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:13,fontWeight:700,color:'var(--text-1)',letterSpacing:'-0.1px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',letterSpacing:'-0.1px'}}>{shop.name}</h3>
                            {shop.visited&&<span style={{fontSize:9,background:'var(--crema)',color:'#FAF6F0',borderRadius:20,padding:'1px 5px',fontWeight:700,flexShrink:0}}>✓</span>}
                          </div>
            ...
            <div style={{flex:1,position:'relative'}}>
              ...
              {isMobile&&!selected&&<button onClick={()=>setActiveView('discover')} style={{position:'absolute',bottom:24,right:16,background:'var(--crema-mid)',color:'#FAF6F0',border:'none',borderRadius:14,padding:'12px 18px',fontWeight:700,fontSize:14,cursor:'pointer',boxShadow:'0 6px 20px rgba(200,150,90,0.4)',zIndex:500,fontFamily:"'DM Sans',sans-serif"}}>☕ Browse All</button>}
              ...
              {selected&&!isMobile&&(
                <div className="animate-slideUp" style={{position:'absolute',bottom:24,left:306,right:24,background:'var(--surface)',border:'1px solid var(--border)',borderRadius:18,padding:'20px',zIndex:500,maxWidth:360}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:12}}>
                    <div><h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:700,color:'var(--text-1)'}}>{selected.name}</h3><p style={{fontSize:11,color:'var(--text-3)',marginTop:2}}>{selected.address}</p></div>
