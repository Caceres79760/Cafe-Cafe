...
function BrowseView({shops,avgScores,onSelect,onLog}) {
  ...
  return (
    <div style={{maxWidth:680,margin:'0 auto',padding:'32px 24px'}}>
      <h1 className="animate-fadeUp" style={{fontFamily:"'Cormorant Garamond',serif",fontSize:38,fontWeight:700,color:'var(--crema)',marginBottom:6}}>Browse</h1>
  ...
  return (
    <div style={{height:'100vh',display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{flex:1,overflow:'hidden',position:'relative'}}>
        ...
        {activeView==='browse'&&<div style={{overflowY:'auto',height:'100%'}}>{loadingShops?<Spinner/>:<BrowseView shops={shopsWithVisited} avgScores={avgScores} onSelect={s=>{setSelected(s);setActiveView('map')}} onLog={handleLog}/>}</div>}

        {activeView==='friends'&&<div style={{overflowY:'auto',height:'100%'}}><FriendsView user={user} shops={shops} onNeedAuth={handleNeedAuth}/></div>}
        {activeView==='profile'&&<div style={{overflowY:'auto',height:'100%'}}><ProfileView user={user} visits={visits} shops={shops} loading={loadingVisits} onAuthClick={()=>setShowAuth(true)}/></div>}
      </div>
