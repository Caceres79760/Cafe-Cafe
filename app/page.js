...
function FeatureCard({ icon, title, desc, delay }) {
  return (
    <div className="animate-fadeUp" style={{
      ...
      <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontWeight: 700, color: C.crema, marginBottom: 8 }}>{title}</h3>
      <p style={{ fontSize: 14, color: C.text2, lineHeight: 1.7 }}>{desc}</p>
    </div>
  )
}
...
function StepCard({ number, title, desc, delay }) {
  return (
    <div className="animate-fadeUp" style={{ animationDelay: delay, display: 'flex', gap: 20, alignItems: 'flex-start', marginBottom: 28 }}>
      ...
      <div>
        <h4 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, fontWeight: 700, color: C.crema, marginBottom: 4 }}>{title}</h4>
        <p style={{ fontSize: 14, color: C.text2, lineHeight: 1.7 }}>{desc}</p>
      </div>
    </div>
  )
  ...
  return (
    <div className="animate-fadeUp" style={{
      ...
      <div style={{ padding: '14px 16px' }}>
        <h4 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, fontWeight: 700, color: C.crema, marginBottom: 2 }}>{name}</h4>
...
function AuthModal({ mode: initialMode, onClose, onSuccess }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,8,5,0.92)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 24 }}
      onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 24, width: '100%', maxWidth: 400, padding: '36px 32px' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 32, fontWeight: 700, fontStyle: 'italic', color: C.crema }}>café café</h1>
        ...
        <button onClick={handleSubmit} disabled={loading} style={{ width: '100%', background: C.cremaMid, color: C.bg, border: 'none', borderRadius: 12, padding: '14px', fontWeight: 700, fontSize: 15, cursor: loading ? 'default' : 'pointer', opacity: loading ? 0.7 : 1, fontFamily: "'DM Sans', sans-serif", transition: 'all 0.2s' }}>
...
export default function LandingPage() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      {/* ── NAV ── */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: 'rgba(15,12,9,0.88)', backdropFilter: 'blur(12px)', borderBottom: `1px solid ${C.border}`, height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontWeight: 700, fontStyle: 'italic', color: C.crema }}>café café</span>
      ...
      <section style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '120px 24px 80px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        {/* Background glow */}
        <div style={{ position: 'absolute', top: '20%', left: '50%', transform: 'translateX(-50%)', width: 600, height: 600, borderRadius: '50%', background: C.cremaMid, opacity: 0.04, filter: 'blur(80px)', pointerEvents: 'none' }} />
        ...
        <div style={{ maxWidth: 720, position: 'relative' }}>
          ...
          {/* Headline */}
          <h1 className="animate-fadeUp delay-100 hero-headline" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(40px, 8vw, 86px)', fontWeight: 700, color: C.crema, lineHeight: 1.05, letterSpacing: -1, marginBottom: 24 }}>
            Your city's coffee,<br />
            <span style={{ color: C.cremaMid, fontStyle: 'italic' }}>finally mapped.</span>
          </h1>

          {/* Subheadline */}
          <p className="animate-fadeUp delay-200" style={{ fontSize: 'clamp(16px, 2.5vw, 20px)', color: C.text2, lineHeight: 1.7, marginBottom: 40, maxWidth: 560, margin: '0 auto 40px' }}>
          ...
          <div className="animate-fadeUp delay-300" style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 48 }}>
            <button onClick={openSignUp} style={{ background: C.cremaMid, color: C.bg, border: 'none', borderRadius: 14, padding: '16px 36px', fontSize: 16, fontWeight: 700, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", transition: 'all 0.2s', boxShadow: `0 8px 32px ${C.cremaMid}44` }}
          ...
          <div className="animate-fadeUp delay-400" style={{ display: 'flex', gap: 32, justifyContent: 'center', flexWrap: 'wrap' }}>
            {[
              ...
              <div key={stat.label} style={{ textAlign: 'center' }}>
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 32, fontWeight: 700, color: C.crema }}>
                  <Counter target={stat.value} suffix={stat.suffix} />
                </div>
                <div style={{ fontSize: 12, color: C.text3, marginTop: 2 }}>{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '0 0 80px', overflow: 'hidden' }}>
        <div style={{ textAlign: 'center', marginBottom: 32, padding: '0 24px' }}>
          <p style={{ fontSize: 12, color: C.cremaMid, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 8 }}>Featured in Rochester</p>
      ...
      <section style={{ padding: '80px 32px', maxWidth: 1000, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <p style={{ fontSize: 12, color: C.cremaMid, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 8 }}>What you get</p>
          <h2 className="animate-fadeUp" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 40, fontWeight: 700, color: C.crema }}>Everything a coffee explorer needs</h2>
        </div>
      ...
      <section style={{ padding: '80px 32px', background: C.surface, borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            <p style={{ fontSize: 12, color: C.cremaMid, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 8 }}>How it works</p>
            <h2 className="animate-fadeUp" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 40, fontWeight: 700, color: C.crema }}>Three steps to a better cup</h2>
          </div>
      ...
      <section style={{ padding: '100px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          ...
          <h2 className="animate-fadeUp" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(36px, 6vw, 56px)', fontWeight: 700, color: C.crema, marginBottom: 16, lineHeight: 1.1 }}>
          ...
          <button className="animate-fadeUp delay-200" onClick={openSignUp} style={{ background: C.cremaMid, color: C.bg, border: 'none', borderRadius: 14, padding: '18px 44px', fontSize: 17, fontWeight: 700, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", transition: 'all 0.2s', boxShadow: `0 8px 40px ${C.cremaMid}44` }}
            onMouseEnter={e => { e.currentTarget.style.background = C.cremaDark; e.currentTarget.style.transform = 'translateY(-2px)' }}
            onMouseLeave={e => { e.currentTarget.style.background = C.cremaMid; e.currentTarget.style.transform = 'translateY(0)' }}>
      ...
      <footer style={{ borderTop: `1px solid ${C.border}`, padding: '28px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, fontWeight: 700, fontStyle: 'italic', color: C.crema }}>café café</span>
