...
// Hand-built SVGs in the brand palette — matching the
// same illustrated style as the map's coffee-cup markers,
// rather than relying on emoji (which render inconsistently
// across platforms and clash with the editorial brand feel).
const FeatureIcons = {
  map: (
    <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
      <path d="M9 4L3 6v14l6-2 6 2 6-2V4l-6 2-6-2Z" stroke="#FAF6F0" strokeWidth="1.6" strokeLinejoin="round" fill="none"/>
      <path d="M9 4v14M15 6v14" stroke="#FAF6F0" strokeWidth="1.6" strokeLinecap="round"/>
      <circle cx="9.5" cy="10.5" r="1.4" fill="#FAF6F0"/>
    </svg>
  ),
  star: (
    <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
      <path d="M12 3.5l2.47 5.18 5.53.74-4.04 3.97 1 5.61L12 16.2l-4.96 2.8 1-5.61-4.04-3.97 5.53-.74L12 3.5Z"
        stroke="#FAF6F0" strokeWidth="1.6" strokeLinejoin="round" fill="none"/>
    </svg>
  ),
  pin: (
    <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
      <path d="M12 21s7-7.2 7-12a7 7 0 1 0-14 0c0 4.8 7 12 7 12Z"
        stroke="#FAF6F0" strokeWidth="1.6" strokeLinejoin="round" fill="none"/>
      <circle cx="12" cy="9" r="2.4" stroke="#FAF6F0" strokeWidth="1.6"/>
    </svg>
  ),
  friends: (
    <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
      <circle cx="8.5" cy="8" r="2.6" stroke="#FAF6F0" strokeWidth="1.6"/>
      <circle cx="16" cy="9" r="2.1" stroke="#FAF6F0" strokeWidth="1.6"/>
      <path d="M3.5 19c.4-3 2.4-4.8 5-4.8s4.6 1.8 5 4.8" stroke="#FAF6F0" strokeWidth="1.6" strokeLinecap="round" fill="none"/>
      <path d="M14.2 14.6c2.1.2 3.7 1.8 4 4.4" stroke="#FAF6F0" strokeWidth="1.6" strokeLinecap="round" fill="none"/>
    </svg>
  ),
}
    ...
    <div className="animate-fadeUp" style={{
      ...
      <div style={{ padding: '14px 16px' }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
          {tags.map(t => <span key={t} style={{ fontSize: 10, color: C.text2, background: C.steam, border: `1px solid ${C.border}`, borderRadius: 5, padding: '2px 7px' }}>{t}</span>)}
        </div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(180,165,145,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 24 }}
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 24, boxShadow: '0 8px 40px rgba(0,0,0,0.12)', width: '100%', maxWidth: 400, padding: '36px 32px' }}>
        ...
        <div style={{ display: 'flex', background: C.steam, borderRadius: 10, padding: 3, marginBottom: 24 }}>
          {['login', 'signup'].map(m => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 16 }}>
          {mode === 'signup' && <input style={{ width: '100%', background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 12, padding: '12px 14px', color: C.crema, fontSize: 14, outline: 'none', fontFamily: "'DM Sans', sans-serif" }} type="text" placeholder="Username (e.g. coffeejuan)" value={username} onChange={e => setUsername(e.target.value.toLowerCase())} onKeyDown={e => e.key === 'Enter' && handleSubmit()} />}
          <input style={{ width: '100%', background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 12, padding: '12px 14px', color: C.crema, fontSize: 14, outline: 'none', fontFamily: "'DM Sans', sans-serif" }} type="email" placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()} />
          <input style={{ width: '100%', background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 12, padding: '12px 14px', color: C.crema, fontSize: 14, outline: 'none', fontFamily: "'DM Sans', sans-serif" }} type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()} />
        </div>
    ...
    <div style={{ background: C.bg, minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      <section style={{ padding: '80px 32px', maxWidth: 1000, margin: '0 auto' }}>
        ...
        <div className="features-grid" style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          <FeatureCard icon={FeatureIcons.map} title="Interactive Map" desc="Every café in Rochester pinned on a live map. See what you've visited, what's near you, and what your friends recommend." delay="0s" />
          <FeatureCard icon={FeatureIcons.star} title="Three-Score Rating" desc="Rate every visit on Ambiance, Taste, and Value — the three things that actually tell you if a place is worth it." delay="0.1s" />
          <FeatureCard icon={FeatureIcons.pin} title="Nearby Discovery" desc="Enable location and instantly see which cafés are a 5-minute walk away, sorted by distance with estimated walk times." delay="0.2s" />
          <FeatureCard icon={FeatureIcons.friends} title="Friends & Profiles" desc="Add friends, see where they've been, and let their ratings guide your next choice. Coffee is better shared." delay="0.3s" />
        </div>
      </section>
        ...
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          <div style={{
            ...
            <svg width="40" height="40" viewBox="0 0 42 42" fill="none">
              {/* Cup body */}
              <path d="M9 15h20l-2 14a4 4 0 0 1-4 3.4H15a4 4 0 0 1-4-3.4L9 15Z"
                fill="#FAF6F0" stroke="#FAF6F0" strokeWidth="1.4" strokeLinejoin="round"/>
              {/* Handle */}
              <path d="M29 18c4 0 6 2.4 6 5.4S33 29 29 29"
                stroke="#FAF6F0" strokeWidth="2.2" fill="none" strokeLinecap="round"/>
              {/* Saucer */}
              <ellipse cx="19" cy="34.5" rx="11" ry="2" fill="#FAF6F0" opacity="0.55"/>
              {/* Steam */}
              <path d="M14 11c-1.5-1.6-1.5-3.2 0-4.8M19 11c-1.5-1.6-1.5-3.2 0-4.8M24 11c-1.5-1.6-1.5-3.2 0-4.8"
                stroke="#FAF6F0" strokeWidth="1.6" strokeLinecap="round" fill="none" opacity="0.85"/>
            </svg>
          </div>
      ...
      <footer style={{ borderTop: `1px solid ${C.border}`, padding: '28px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <a href="/contact" style={{ background: 'none', border: 'none', color: C.text3, fontSize: 12, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", textDecoration: 'none' }}>
            Contact Us
          </a>
