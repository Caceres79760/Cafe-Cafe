...
function SearchView({shops,avgScores,onSelect,onLog}) {
  ...
  const filtered=shops.filter(shop=>{
    ...
    return(!query||shop.name.toLowerCase().includes(query.toLowerCase())||shop.neighborhood?.toLowerCase().includes(query.toLowerCase())||shop.tags?.some(t=>t.toLowerCase().includes(query.toLowerCase())))&&(neighborhood==='all'||shop.neighborhood===neighborhood)&&(tag==='all'||shop.tags?.includes(tag))&&(minScore===0||avg>=minScore)
  })
  return (
    <div style={{maxWidth:680,margin:'0 auto',padding:'32px 24px'}}>
      ...
      <div className="animate-fadeUp delay-200" style={{display:'flex',gap:10,marginBottom:24,flexWrap:'wrap'}}>
        <select value={neighborhood} onChange={e=>setNeighborhood(e.target.value)} style={{background:'var(--surface-2)',border:`1.5px solid ${neighborhood!=='all'?'var(--crema-mid)':'var(--border)'}`,borderRadius:10,color:neighborhood!=='all'?'var(--crema)':'var(--text-3)',padding:'8px 12px',fontSize:13,cursor:'pointer',outline:'none',fontFamily:"'DM Sans',sans-serif"}}>{neighborhoods.map(n=><option key={n} value={n} style={{background:'var(--surface-2)'}}>{n==='all'?'All Neighbourhoods':n}</option>)}</select>
        <select value={tag} onChange={e=>setTag(e.target.value)} style={{background:'var(--surface-2)',border:`1.5px solid ${tag!=='all'?'var(--crema-mid)':'var(--border)'}`,borderRadius:10,color:tag!=='all'?'var(--crema)':'var(--text-3)',padding:'8px 12px',fontSize:13,cursor:'pointer',outline:'none',fontFamily:"'DM Sans',sans-serif"}}>{allTags.map(t=><option key={t} value={t} style={{background:'var(--surface-2)'}}>{t==='all'?'All Tags':t}</option>)}</select>
        ...
        {(query||neighborhood!=='all'||tag!=='all'||minScore>0)&&<button onClick={()=>{setQuery('');setNeighborhood('all');setTag('all');setMinScore(0)}} style={{background:'none',border:'1px solid var(--border)',borderRadius:10,color:'var(--text-3)',padding:'8px 14px',fontSize:13,cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}}>Clear ✕</button>}
      </div>
      ...
      {filtered.length===0?(<div className="card" style={{padding:'48px',textAlign:'center'}}><div style={{fontSize:40,marginBottom:12}}>🔍</div><p style={{color:'var(--crema)',fontSize:16,fontFamily:"'Cormorant Garamond',serif",marginBottom:6}}>No cafés found</p><p style={{color:'var(--text-3)',fontSize:13}}>Try adjusting your filters.</p></div>)
      :filtered.map((shop,i)=>{
        const avg=avgScores[shop.id]?(avgScores[shop.id].ambiance+avgScores[shop.id].taste+avgScores[shop.id].value)/3:0
        return (
          <div key={shop.id} className="card animate-fadeUp" style={{marginBottom:12,overflow:'hidden',cursor:'pointer',transition:'border-color 0.2s',animationDelay:`${i*0.05}s`}}
            ...
            <div style={{display:'flex'}}>
              ...
              <div style={{padding:'14px 16px',flex:1}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8}}>
                  <div><h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:700,color:'var(--crema)'}}>{shop.name}</h3><p style={{fontSize:11,color:'var(--text-3)',margin:'2px 0 8px'}}>{shop.neighborhood}</p></div>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>{shop.tags?.slice(0,2).map(t=><span key={t} style={{fontSize:10,color:'var(--text-3)',background:'var(--surface-3)',border:'1px solid var(--border)',borderRadius:5,padding:'2px 7px'}}>{t}</span>)}</div>
                  <div style={{display:'flex',alignItems:'center',gap:6}}>{avg>0&&<Stars value={Math.round(avg)} size={12}/>}<button onClick={e=>{e.stopPropagation();onLog(shop)}} className="btn-primary" style={{fontSize:11,padding:'5px 12px'}}>Log</button></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div key={shop.id} className="card animate-fadeUp" style={{marginBottom:16,overflow:'hidden',cursor:'pointer',transition:'border-color 0.2s',animationDelay:`${i*0.07}s`}}
            ...
            <div style={{padding:'16px 20px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                <div><h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:700,color:'var(--crema)'}}>{shop.name}</h3><p style={{fontSize:12,color:'var(--text-3)',marginTop:2}}>{shop.address} · {shop.neighborhood}</p></div>
              ...
              <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:10}}>{shop.tags?.map(t=><span key={t} style={{fontSize:11,color:'var(--text-2)',background:'var(--surface-3)',border:'1px solid var(--border)',borderRadius:6,padding:'2px 8px'}}>{t}</span>)}</div>
...
function VisitedView({visits,shops,loading}) {
  ...
  return (
    <div style={{maxWidth:680,margin:'0 auto',padding:'32px 24px'}}>
      ...
      {visits.length===0?(<div className="card animate-fadeUp" style={{padding:'48px',textAlign:'center'}}><div style={{fontSize:48,marginBottom:12}}>☕</div><p style={{color:'var(--crema)',fontSize:18,fontFamily:"'Cormorant Garamond',serif",marginBottom:8}}>No visits yet</p><p style={{color:'var(--text-3)',fontSize:13}}>Head to the map and start exploring!</p></div>)
      :visits.map((visit,i)=>{
        ...
        return (
          <div key={visit.id} className="card animate-fadeUp" style={{marginBottom:12,padding:'16px',animationDelay:`${i*0.07}s`}}>
            <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
              ...
              <div style={{flex:1}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                  <div><h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:700,color:'var(--crema)'}}>{shop.name}</h3><p style={{fontSize:11,color:'var(--text-3)',margin:'2px 0 6px'}}>{shop.neighborhood} · {new Date(visit.visited_at).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}</p></div>
  ...
  return (
    <div style={{background:P.bg,minHeight:'100%'}}>
      <div style={{maxWidth:640,margin:'0 auto',padding:'40px 24px 60px'}}>
        ...
        {visits.length===0&&(<div style={{background:P.surface,border:`1px solid ${P.border}`,borderRadius:18,padding:'48px',textAlign:'center'}}><div style={{fontSize:48,marginBottom:12}}>☕</div><p style={{color:P.espresso,fontSize:18,fontFamily:"'Cormorant Garamond',serif",marginBottom:8}}>No visits yet</p><p style={{color:P.slate,fontSize:13}}>Start logging your café visits to build your profile.</p></div>)}
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{height:'100vh',display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{flex:1,overflow:'hidden',position:'relative'}}>
        ...
        {activeView==='nearby'&&<div style={{overflowY:'auto',height:'100%'}}>{loadingShops?<Spinner/>:<NearbyView shops={shopsWithVisited} avgScores={avgScores} userLocation={userLocation} locationError={locationError} locationLoading={locationLoading} onRequestLocation={requestLocation} onSelect={s=>{setSelected(s);setActiveView('map')}} onLog={handleLog}/>}</div>}
        {activeView==='discover'&&<div style={{overflowY:'auto',height:'100%'}}>{loadingShops?<Spinner/>:<DiscoverView shops={shopsWithVisited} avgScores={avgScores} onSelect={s=>{setSelected(s);setActiveView('map')}} onLog={handleLog}/>}</div>}
        {activeView==='search'&&<div style={{overflowY:'auto',height:'100%'}}>{loadingShops?<Spinner/>:<SearchView shops={shopsWithVisited} avgScores={avgScores} onSelect={s=>{setSelected(s);setActiveView('map')}} onLog={handleLog}/>}</div>}
        {activeView==='visited'&&<div style={{overflowY:'auto',height:'100%'}}>{!user?(<div style={{maxWidth:480,margin:'80px auto',padding:'0 24px',textAlign:'center'}}><div style={{fontSize:52,marginBottom:16}}>☕</div><h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,color:'var(--crema)',marginBottom:8}}>Sign in to see your visits</h2><p style={{fontSize:14,color:'var(--text-3)',marginBottom:24}}>Create a free account to track every café you visit.</p><button onClick={()=>setShowAuth(true)} className="btn-primary" style={{fontSize:15,padding:'13px 32px'}}>Sign In / Sign Up</button></div>):<VisitedView visits={visits} shops={shops} loading={loadingVisits}/>}</div>}
        {activeView==='friends'&&<div style={{overflowY:'auto',height:'100%'}}><FriendsView user={user} shops={shops} onNeedAuth={handleNeedAuth}/></div>}
        {activeView==='profile'&&<div style={{overflowY:'auto',height:'100%'}}><ProfileView user={user} visits={visits} shops={shops} onAuthClick={()=>setShowAuth(true)}/></div>}
      </div>
